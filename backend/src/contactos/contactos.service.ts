...
export class ContactosService {
  ...
  async crear(dto: Partial<Contacto>): Promise<Contacto> {
    this.validarCodPaisOpcional(dto.codPais);
    return this.repo.save(this.repo.create(dto));
  }

  async listar(params: { tipo?: string; q?: string; page?: number; limit?: number }) {
    const { tipo, q, page = 1, limit = 20 } = params;
  ...
  async buscar(q: string): Promise<Contacto[]> {
    if (!q || q.length < 2) return [];
    const term = `%${q.toLowerCase()}%`;
    return this.repo.createQueryBuilder('c')
      .where('LOWER(c.nombre) LIKE :term', { term })
      .orWhere('LOWER(c.nit) LIKE :term', { term })
      .orWhere('LOWER(c.numDocumento) LIKE :term', { term })
  ...
  async actualizar(id: string, dto: Partial<Contacto>): Promise<Contacto> {
